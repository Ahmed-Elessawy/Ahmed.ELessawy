...
def extractFemaleNames(filename,letter):
  ...
  Cont=File.read()
  File.close()
  Reg = re.findall('<tr align="right"><td>(\w+)<\/td><td>\w+<\/td><td>(\w+)',Cont)
  
  List = []
  
  for var in Reg:
    if(var[1][0] == letter):
        List.append(var[1] + ' '+var[0])
...
def extractDocAndPopularityDate(filename):
  ...
  Cont=File.read()
  File.close()
  Reg = re.findall('\w+.\w+,.\d\d\d\d',Cont)
  Reg1=re.findall('<h3 align="center">(\w+\s\w+\s\w+)',Cont)
  Res= Reg[0]+" and "+Reg1[0]
  return Res
