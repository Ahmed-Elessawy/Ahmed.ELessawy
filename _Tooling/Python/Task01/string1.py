...
def donuts(count):
  ...
  if 1<=count<15:
    ...
    Result='Number of donuts: too much donuts'
  else:
...
def both_ends(s):
  if len(s)<2:
    ...
    return s[0:2]+s[len(s)-2:]   # concatenation of the first two chars and last two chars of the string
...
def fix_start(s,filler):
  ...
  result=s[0]              #var to store the new edited string and adding the first char that won't be changed
  for i in range(1,len(s)): 
    if s[i] !=s[0]:         
      result=result+s[i]   #stroing the chars in the new empty string if it is not = to the first  char of string
    else:
      result=result+filler #adding filler if the char is equal to the first char
...
def mix_up(a, b):
  # +++your code here+++
  temp=a[0:2]  #temporary vriable to be able to chnge b [0:2] by a[0:2] after changing a
  
  return a.replace(a[0:2],b[0:2]) +"-" +b.replace(b[0:2],temp)  #conacatenation of a and b after replacing first two chars of both strings
